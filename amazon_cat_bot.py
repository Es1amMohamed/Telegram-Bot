...
async def expand_url(url):
    if "amazon." in url.lower() and "/dp/" in url.lower():
        return url
    try:
...
def clean_discount_text(discount_text: str) -> str:
    if not discount_text:
        return ""

    cleaned = ''.join(c for c in discount_text if c.isdigit() or c in ['-', '%'])
    ...
    if len(cleaned) > 6 or cleaned in ['-%', '-0%', '0%']:
        return ""
        
    return cleaned


def calculate_discount_percentage(current: str, old: str) -> str:
    try:
        curr_clean = ''.join(c for c in current if c.isdigit() or c == '.')
        old_clean = ''.join(c for c in old if c.isdigit() or c == '.')
        
        curr_val = float(curr_clean) if curr_clean else 0
        old_val = float(old_clean) if old_clean else 0
        
        if old_val <= 0 or curr_val >= old_val:
            return ""
            
        percentage = round(((old_val - curr_val) / old_val) * 100)
...
async def parse_product_egypt(page):
    return await page.evaluate("""() => {
        ...
        let current = getPrice('.priceToPay .a-offscreen') || getPrice('.apexPriceToPay .a-offscreen') || getPrice('#price_inside_buybox') || getPrice('.a-price-whole');
        let old = getPrice('.basisPrice .a-offscreen') || getPrice('.a-text-price .a-offscreen');
        let disc = document.querySelector('.savingsPercentage')?.innerText.trim() || document.querySelector('.reinventPriceSavingsPercentageMargin')?.innerText.trim() || "";
        return {
...
async def parse_product_saudi(page):
    return await page.evaluate("""() => {
        ...
        let current = !isOut ? (getPrice('.priceToPay .a-offscreen') || getPrice('.apexPriceToPay .a-offscreen') || getPrice('.a-price-whole') || getPrice('#price_inside_buybox')) : "";
        let old = getPrice('.basisPrice .a-offscreen') || getPrice('.a-text-price .a-offscreen');
        let disc = document.querySelector('.savingsPercentage')?.innerText.trim() || document.querySelector('.reinventPriceSavingsPercentageMargin')?.innerText.trim() || "";
        return {
            title: document.querySelector("#productTitle")?.innerText.trim() || "",
...
async def parse_product_uae(page):
    return await page.evaluate("""() => {
        ...
        let currentPriceStr = "";
        if (!isOut) {
            const priceContainer = document.querySelector(".priceToPay span[aria-hidden='true']") || document.querySelector(".apex_on_twister_price span[aria-hidden='true']");
            if (priceContainer) {
        ...
        let oldPriceStr = get(".basisPrice .a-offscreen") || get(".a-text-price[data-a-strike='true'] .a-offscreen") || get("#listPrice");
        let discount = get(".savingsPercentage") || get(".reinventPriceSavingsPercentageMargin") || get(".priceBlockSavingsString");
        if (!discount) {
            let currNum = cleanToNum(currentPriceStr);
            let oldNum = cleanToNum(oldPriceStr);
            if (currNum && oldNum && oldNum > currNum) {
                discount = "-" + Math.round(((oldNum - currNum) / oldNum) * 100) + "%";
            }
        }
...
async def parse_product_usa(page):
    return await page.evaluate('''() => {
        ...
        let oldPrice = get(".basisPrice .a-offscreen") || 
                       get(".a-text-price .a-offscreen") || 
                       get(".a-text-strike") || "";
        ...
        if (!discount && currentPrice && oldPrice) {
            const currNum = parseFloat(currentPrice.replace(/[^0-9.]/g, '')) || 0;
            const oldNum  = parseFloat(oldPrice.replace(/[^0-9.]/g, '')) || 0;
            
            if (oldNum > currNum && oldNum > 0) {
                const percentage = Math.round(((oldNum - currNum) / oldNum) * 100);
...
async def parse_product_europe_common(page):
    return await page.evaluate('''() => {
        ...
        let currentPrice = extractPrice('.priceToPay') || extractPrice('.apexPriceToPay') || extractPrice('.a-price:not(.a-text-price) span[aria-hidden="true"]') || extractPrice('.a-price .a-offscreen');
        let oldPrice = extractPrice(".basisPrice .a-offscreen") || extractPrice(".a-text-price .a-offscreen") || extractPrice(".a-text-strike");
        let discount = get(".savingsPercentage") || get(".reinventPriceSavingsPercentageMargin") || "";
        if (!discount && currentPrice && oldPrice) {
            const curr = parseFloat(currentPrice);
            const old = parseFloat(oldPrice);
            if (old > curr) discount = "-" + Math.round(((old - curr) / old) * 100) + "%";
        } else if (discount && !discount.startsWith('-')) discount = '-' + discount;
        return {
            title: get("#productTitle"),
            img: document.querySelector("#landingImage")?.src || document.querySelector("#imgBlkFront")?.src || "",
            currentPrice: currentPrice || "",
...
async def parse_list_common(page, base_url):
    ...
    let items = Array.from(document.querySelectorAll(['.s-result-item[data-asin]', '.s-card-container', '.p13n-sc-uncoverable-faceout', '.s-product-image-container', '.puis-card-container'].join(', ')));
    for (let item of items) {
        ...
        let title = "";
        const titleSelectors = ['h2.a-text-normal', '.p13n-sc-truncate-desktop-type2', 'h2 a span', 'h2 span', '.a-size-base-plus.a-color-base.a-text-normal'];
        for (let s of titleSelectors) {
            let el = item.querySelector(s);
            if (el) {
                let text = el.innerText.trim();
                if (text.length > 15) { title = text; break; } else if (text.length > 0) title = text;
            }
        }
        if (title.length < 15) {
            const ariaEl = item.querySelector('h2[aria-label]');
            if (ariaEl && ariaEl.getAttribute('aria-label').length > title.length) title = ariaEl.getAttribute('aria-label');
        }
        title = title.replace(/\\n/g, ' ').trim();
        ...
        let currentPrice = "";
        const priceSelectors = ['.a-price:not(.a-text-price) span[aria-hidden="true"]', '.a-price .a-offscreen', '._cDEzb_p13n-sc-price_3mJ9Z', '.a-color-price'];
...
async def scrape_page(url, country):
    ...
    async with async_playwright() as p:
        ...
        await context.add_cookies([{'name': 'i18n-prefs', 'value': currency, 'domain': f".{domain}", 'path': '/'}])
        ...
        try:
            ...
            if product_title_el:
                parser_func = PARSERS_MAP.get(country, parse_product_europe_common)
...
def get_target_lang(context):
    if context.user_data.get("display_lang") == "en":
        return "en"

    country = context.user_data.get("country", "United States")
    country_lang = COUNTRIES_CONFIG.get(country, {}).get("lang", "en")
...
def translate_text(text, target):
    try:
        if not text or detect(text) == target:
            return text
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data.pop("display_lang", None)
    ...
    welcome_msg = (
        "👋 Welcome!\n"
        "Send me an Amazon product, category, or best sellers link\n\n"
        "You can change country and language anytime ↓"
    )

    await update.message.reply_text(
        welcome_msg,
        reply_markup=main_keyboard(context)
    )
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == txt["home"]:
        ...
        kb = [[InlineKeyboardButton(f"{v['flag']} {k}", callback_data=f"c_{k}")] 
              for k, v in COUNTRIES_CONFIG.items()]
        await update.message.reply_text("Choose your country:", reply_markup=InlineKeyboardMarkup(kb))
        ...
        if country_lang != "en":
            lang_name = "العربية" if country_lang == "ar" else country_lang.upper()
            buttons.append([InlineKeyboardButton(f"{lang_name} (local) 🌐", callback_data="lang_local")])

        kb = InlineKeyboardMarkup(buttons)
        await update.message.reply_text("Choose display language:", reply_markup=kb)
...
async def handle_amazon(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for c_name, c_info in COUNTRIES_CONFIG.items():
        if c_info["domain"] in final_url:
            country = c_name
            context.user_data["country"] = c_name
            break

    config = COUNTRIES_CONFIG[country]
    products = await scrape_page(final_url, country)
    ...
    for data in products:
        ...
        caption += f"\n📍 {config['flag']} {local_country_name}\n🔗 <a href='{data['product_url']}'>Product Link</a>"

        if data.get("img"):
            media_group.append(InputMediaPhoto(media=data["img"], caption=caption, parse_mode="HTML"))

    if media_group:
        try:
            await update.message.reply_media_group(media=media_group)
        except:
            await update.message.reply_text("❌ Error sending the data.")
    else:
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data == "lang_local":
        context.user_data["display_lang"] = "local" 

    txt = get_ui_text(context)
    
    await query.edit_message_text(
        f"{txt['updated']}\n\nاخترت: {COUNTRIES_CONFIG.get(context.user_data.get('country', 'United States'))['flag']} {context.user_data.get('country')}",
        reply_markup=None  
    )
